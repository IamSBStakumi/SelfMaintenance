...
- **[Blocker] Server Component から直接 `onClick` 等のイベントハンドラを渡していないか**
...
- **[Warning] データフェッチが Client Component 側で行われていないか（Server Component で行えないか検討）**
  初期データの取得は Server Component で行い、インタラクティブな再取得は TanStack Query で行う構成を推奨する。

- **[Suggestion] `"use client"` の境界はできるだけ末端（葉）のコンポーネントに置いているか**
...
- **[Blocker] `any` 型を使用していないか**
  `any` は型安全性を破壊する。`unknown` を使い、型ガードで絞り込むこと。

- **[Blocker] Supabase のレスポンスをノーチェックで使用していないか**
...
- **[Warning] Zod スキーマによるバリデーションが欠けていないか**
  外部入力（フォーム値・API レスポンス等）は必ず Zod でバリデーションすること。`lib/` 配下にスキーマが定義されているか確認する。

- **[Warning] `as` によるキャストを多用していないか**
  型キャストは型推論が困難な箇所にとどめ、理由をコメントで明記すること。

### React / フック

- **[Blocker] `useEffect` 内で非同期処理を直接 `await` していないか**
...
- **[Warning] `useEffect` の依存配列に漏れがないか**
  ESLint の `exhaustive-deps` ルールに従い、依存配列を正確に記述すること。

- **[Warning] カスタムフックの責務が肥大化していないか**
  1つのカスタムフックが UI ロジック・データ取得・バリデーションを全て担っている場合は分割を検討する。

- **[Suggestion] TanStack Query の `queryKey` は一元管理されているか**
  キャッシュの整合性のため、`queryKey` は `lib/` や `hooks/` に定数として切り出すことを推奨する。

### 命名規則

- **[Warning] コンポーネント名はパスカルケース（`PascalCase`）か**
- **[Warning] カスタムフックは `use` プレフィックスで始まるか（例: `useTaskList`）**
- **[Suggestion] 型名は `PascalCase` で、インターフェースには `I` プレフィックスを付けない**
...
- **[Blocker] 実際の Supabase / 外部 API に接続するテストが含まれていないか**
  `vi.mock()` でモックし、ユニットテストは外部依存なしで完結すること。

- **[Blocker] テストが実装の内部実装（プライベート関数・状態の直接参照）に依存していないか**
  Testing Library の原則に従い、ユーザーが操作する形でテストを書くこと（`getByRole`, `getByText` 等を優先）。

- **[Warning] テストケースが正常系のみでなく、異常系・境界値もカバーしているか**
...
- **[Warning] `vi.mock()` のリセットが適切に行われているか**
  `beforeEach` で `vi.clearAllMocks()` または `vi.resetAllMocks()` を呼び出し、テスト間の副作用を防ぐこと。

- **[Suggestion] テストデータ（フィクスチャ）はヘルパー関数またはファクトリに切り出しているか**
  `createMockTask()` のようなヘルパーを `src/test/` 配下に定義し、各テストで再利用すること。

- **[Suggestion] Storybook のストーリーはコンポーネントの主要な状態（通常・エラー・ローディング）を網羅しているか**
